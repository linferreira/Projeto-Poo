...
public class DaoMatricula {
    ...
    public void alterar(Matricula matricula) {
        PreparedStatement ps = null;
        try {
            ps = conn.prepareStatement("UPDATE tb_Matricula SET codAvista = ?, codAprazo = ?"
                    + "WHERE cpf = ?");

            if (matricula.getaVista() == null) {
                ps.setString(1, null);
                ps.setInt(2, matricula.getaPrazo().getCodAprazo());
            } else {
                ps.setInt(1, matricula.getaVista().getCodAvista());
                ps.setString(2, null);
            }
